...
deploy_stacks() {
    ...
    for i in $(eval echo "{${_start}..${_end}..${_increment}}");
    do
...
delete_stacks() {
    ...
    for i in $(eval echo "{${_length}..0..-1}");
    do
...
add_cw_log_group_tags() {
  ...
  let _length=_length-1
  for s_counter in `seq 0 ${_length}`;
  do
...
rolling_restart() {
  ...
  while [ "${_v_wait_rr_rc}" == "255" ] && [ ${retry_count} -lt 4 ]
  do
